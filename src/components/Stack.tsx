...
export default function Stack({
  ...
  const [stack, setStack] = useState<StackItem[]>(() => {
    // Initial state needs to be deterministic for SSR (rotation = 0)
    ...
    return initialCards.map((content, index) => ({
      id: index + 1,
      content: React.isValidElement(content) ? content : (content as any).content || content,
      rotation: 0 // Start with 0 for SSR match
    }));
  });

  // Apply random rotation on client-side mount
  useEffect(() => {
    if (randomRotation) {
  ...
  }, [randomRotation]); // Trigger when randomRotation changes (or on mount)
